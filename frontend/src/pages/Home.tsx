...
export default function Home() {
  ...
  return (
    <div>
      {/* Hero Section */}
      <section className="relative py-20 bg-gradient-to-r from-primary-500 to-primary-700 text-white rounded-lg overflow-hidden mb-12">
        <div className="absolute inset-0 opacity-10 flex items-center justify-end pr-10">
          <Dna size={200} />
        </div>
        <div className="relative z-10 max-w-2xl">
          <h1 className="text-5xl font-bold font-display mb-4">Discover the World of Dinosaurs</h1>
          <p className="text-lg text-gray-100 mb-6">
            Explore prehistoric creatures in stunning 3D, test your knowledge with quizzes, and unlock premium content!
          </p>
          <div className="flex gap-4">
            <Link to="/encyclopedia" className="px-6 py-3 bg-accent-400 text-black font-semibold rounded hover:bg-yellow-400 transition">
              Explore Dinosaurs
            </Link>
            <Link to="/support" className="px-6 py-3 bg-white text-primary-700 font-semibold rounded hover:bg-gray-100 transition flex items-center gap-2">
      ...
      <section className="mb-16">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-3xl font-bold font-display text-primary">Featured Dinosaurs</h2>
          {featuredDinos.length > 3 && (
            <div className="flex gap-2">
              ...
              <button
                onClick={() => setCarouselIndex(Math.min(featuredDinos.length - 3, carouselIndex + 1))}
          ...
          <div className="overflow-hidden">
            ...
            >
              {featuredDinos.map(dino => (
                <Link key={dino.id} to={`/dino/${dino.id}`} className="min-w-[calc(33.333%-1rem)] flex-shrink-0">
                  <div className="bg-gray-800 rounded-lg shadow hover:shadow-lg transition overflow-hidden h-full group">
                    ...
                    <div className="p-4">
                      <h3 className="text-xl font-semibold font-display text-primary mb-1">{dino.name}</h3>
      ...
      <section className="mb-16 bg-gradient-to-r from-gray-800 to-gray-700 rounded-2xl p-8">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold font-display text-primary mb-2">Unlock Premium Features</h2>
          <p className="text-gray-300">Get full access to all dinosaur content with a Premium subscription</p>
        </div>
      ...
      {/* CTA Section */}
      <section className="bg-accent-500 text-white rounded-lg p-12 text-center mb-12">
        <h2 className="text-3xl font-bold font-display mb-4">Ready to Learn More?</h2>
        <p className="text-lg mb-6 max-w-2xl mx-auto">
          Take our interactive dinosaur quiz to test your knowledge and track your progress!
        </p>
        <Link to="/quiz" className="px-8 py-3 bg-white text-accent-500 font-semibold rounded hover:bg-gray-100 transition inline-block">
          Start Quiz
        </Link>
      </section>
