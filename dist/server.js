...
app.post("/api/dinosaurs", async (req, res) => {
    try {
        const { name, species, period, diet, habitat, lengthMeters, weightKg, description, modelUrl, roarSound, imageUrl, imageUrl1, imageUrl2, imageUrl3, imageUrl4, imageUrl5, videoUrl, taxonomy, type, livedIn, namedBy } = req.body;
        if (!name || !species) {
...
app.get("/api/analytics", authMiddleware, adminMiddleware, async (_req, res) => {
    try {
        ...
        // Get counts
        const [totalUsers, totalDinosaurs, totalQuizzes, premiumSubscribers, usersLast7Days, usersLast30Days, quizCompletions, recentScores,] = await Promise.all([
            prisma.user.count(),
...
app.get("/api/favorites/check/:dinoId", authMiddleware, async (req, res) => {
    try {
...
app.post("/api/favorites", authMiddleware, async (req, res) => {
    try {
        ...
        if (!id)
            return res.status(400).json({ error: "dinoId or dinosaurId required" });
...
app.delete("/api/favorites/:dinoId", authMiddleware, async (req, res) => {
    try {
        const { dinoId } = req.params;
...
app.post("/api/user/profile-pic", authMiddleware, upload.single("profilePic"), async (req, res) => {
    try {
        if (!req.file) {
        ...
        if (oldUser?.profilePic) {
            const oldPath = path_1.default.join(uploadsDir, path_1.default.basename(oldUser.profilePic));
...
app.delete("/api/user/profile-pic", authMiddleware, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        if (user?.profilePic) {
            ...
            const filePath = path_1.default.join(uploadsDir, path_1.default.basename(user.profilePic));
...
function buildDinoPrompt(userMessage) {
...
function formatDinoResponse(aiResponse, originalQuestion) {
...
async function getLocalDinoResponse(message) {
    ...
    const matchedDino = dinos.find(d => (d.name && msg.includes(d.name.toLowerCase())) ||
